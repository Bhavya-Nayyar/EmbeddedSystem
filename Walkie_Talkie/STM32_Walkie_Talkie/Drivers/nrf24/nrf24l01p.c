...
static void write_register_multi(uint8_t reg, uint8_t *data, uint8_t length) {
  ...
  HAL_SPI_Transmit(NRF24L01P_SPI, data, length, 2000);
...
void nrf24l01p_rx_init(channel MHz, air_data_rate bps) {
   ...
   *
   * IMPORTANT:
   * This must match the ESP32 configuration.
   */
  ...
  /*
   * Auto acknowledgement on pipe 0.
   */
  ...
  /*
   * nRF24 needs time to enter standby.
   */
  HAL_Delay(2);
...
{
  /*
   * nRF24 power-on/reset settling time
   */
  HAL_Delay(15);
...
void nrf24l01p_set_rx_address(uint8_t *address) {
...
bool nrf24l01p_data_ready(void) {
